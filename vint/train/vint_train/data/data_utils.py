...
def yaw_rotmat(curr_yaw):
    """
    Computes a rotation matrix for a given yaw angle (curr_yaw).
    Supports both 2D and 3D formats.

    Args:
        curr_yaw: A scalar, 2D, or 3D numpy array representing the yaw angle.

    Returns:
        numpy array: Rotation matrix based on the yaw angle.
    """
    # Ensure curr_yaw is a numpy array
    curr_yaw = np.array(curr_yaw)

    if curr_yaw.ndim == 0:  # Scalar value
        curr_yaw = float(curr_yaw)
        return np.array(
            [
                [np.cos(curr_yaw), -np.sin(curr_yaw), 0],
                [np.sin(curr_yaw), np.cos(curr_yaw), 0],
                [0, 0, 1],
            ]
        )
    elif curr_yaw.ndim == 1:  # 2D format
        if curr_yaw.shape[0] == 1:
            curr_yaw = float(curr_yaw[0])
            return np.array(
                [
                    [np.cos(curr_yaw), -np.sin(curr_yaw), 0],
                    [np.sin(curr_yaw), np.cos(curr_yaw), 0],
            ...
            raise ValueError(f"Unexpected 2D format for yaw: {curr_yaw}")
    elif curr_yaw.ndim == 2:  # 3D format
        if curr_yaw.shape == (1, 1):
            curr_yaw = float(curr_yaw[0, 0])
            return np.array(
                [
                    [np.cos(curr_yaw), -np.sin(curr_yaw), 0],
                    [np.sin(curr_yaw), np.cos(curr_yaw), 0],
            ...
            raise ValueError(f"Unexpected 3D format for yaw: {curr_yaw.shape}")
    else:
        raise ValueError(f"Unsupported yaw format with ndim={curr_yaw.ndim}")
